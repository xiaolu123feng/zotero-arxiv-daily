...
def rerank_paper(candidate:list[ArxivPaper],corpus:list[dict],model:str='avsolatorio/GIST-small-Embedding-v0') -> list[ArxivPaper]:
    ...
    corpus = [p for p in corpus if p['data'].get('abstractNote') and p['data']['abstractNote'].strip()]
    candidate = [c for c in candidate if getattr(c, 'summary', None) and c.summary.strip()]
